...
class ClaudeAgentService:
    ...
    async def _build_auth_env(
        ...
        elif provider == ModelProvider.MINIMAX and user_settings.minimax_api_key:
            env["ANTHROPIC_AUTH_TOKEN"] = user_settings.minimax_api_key
    ...
    def _build_minimax_servers(self, minimax_api_key: str) -> dict[str, Any]:
        """构建 MiniMax MCP 服务器配置"""
        return {
            "minimax-mcp-server": self._npx_server_config(
                "@minimax/mcp-server",
                env={
    ...
    async def _get_mcp_servers(
        ...
        if use_minimax_mcp and user_settings.minimax_api_key:
            servers.update(self._build_minimax_servers(user_settings.minimax_api_key))
